...
Walker.prototype.parseSymref = function parseSymref(str) {
  const match = str.match(/^ref:\s+([^\s]*)\s*$/);
  if (match === null)
...
Walker.prototype.visit = function visit(ref, onObject, callback) {
  this.resolve(ref, (err, hash) => {
    if (err)
      return callback(err);

    this._walk(hash, onObject, callback);
  });
};
...
Walker.prototype._walk = function _walk(hash, onObject, callback) {
  this.getObject(hash, (err, header, body) => {
    ...
    body = copyBody(body, (err, body) => {
      ...
      const tree = match[1];
      this._walkTree(tree, onObject, callback);
    });

    onObject(header, body);
  });
};
...
Walker.prototype._walkTree = function _walkTree(hash, onObject, callback) {
  const next = (entity, callback) => {
    if (entity.mode === 40000)
      return this._walkTree(entity.hash, onObject, callback);
    else if (entity.mode === 160000)
      return this._walkSubmodule(entity.hash, onObject, callback);
    else
      return this._walkBlob(entity.hash, onObject, callback);
  };
...
Walker.prototype._walkBlob = function _walkBlob(hash, onObject, callback) {
  this.getObject(hash, (err, header, body) => {
    if (err)
      return callback(err);
