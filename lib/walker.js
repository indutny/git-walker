...
Walker.prototype.parseSymref = function parseSymref(str) {
  ...
  return path.join('objects', match[1].slice(0, 2), match[1].slice(2));
};
...
Walker.prototype.resolveRef = function resolveRef(ref, callback) {
  ...
  const match = ref.match(/^objects\/([a-z0-9]{2})\/([a-z0-9]{38})/);
  if (match !== null)
    return callback(null, match[1] + match[2]);
...
Walker.prototype.getLooseObject = function getLooseObject(hash, callback) {
  ...
  const onReadable = () => {
    ...
    header = header.split(/ /);
    callback(null, header[0], header[1], inflate);
  };
...
Walker.prototype.getObject = function getObject(hash, callback) {
  this.getPackedObject(hash, (err, type, size, body) => {
    if (!err)
      return callback(null, type, size, body);
...
Walker.prototype.visit = function visit(ref) {
  ...
  async.waterfall([
    (callback) => {
      this.resolve(ref, callback);
    },
    (hash, callback) => {
      this._visitCommitOrTag('', hash, res, callback);
    }
  ], (err) => {
    if (err)
...
Walker.prototype._visitCommitOrTag = function _visitCommitOrTag(dir, hash,
                                                                stream,
                                                                callback) {
  this.getObject(hash, (err, type, size, body) => {
    if (err)
      return callback(err);
    assert(type === 'tag' || type === 'commit', `Unexpected type ${type}`);
    ...
    body = copyBody(body, (err, body) => {
      ...
      const match = body.toString().match(
          type === 'commit' ? /(?:^|[\r\n])tree ([a-z0-9]{40})/ :
                              /(?:^|[\r\n])object ([a-z0-9]{40})/);
      if (match === null)
        return callback(new Error(`Failed to parse ${type} body: ${body}`));

      if (type === 'commit')
        this._visitTree(dir, match[1], stream, callback);
      else
        this._visitCommitOrTag(dir, match[1], stream, callback);
    });

    if (!stream.write(new GitObject(dir, hash, type, size, body)))
...
Walker.prototype._visitTree = function _visitTree(dir, hash, stream, callback) {
  const next = (entity, callback) => {
    const nextDir = dir === '' ? entity.name : dir + '/' + entity.name;
    if (entity.mode === 40000)
      return this._visitTree(nextDir, entity.hash, stream, callback);
    else if (entity.mode === 160000)
      return this._visitSubmodule(nextDir, entity.hash, stream, callback);
    else
      return this._visitBlob(nextDir, entity.hash, stream, callback);
  };
...
Walker.prototype._visitBlob = function _visitBlob(dir, hash, stream, callback) {
  this.getObject(hash, (err, type, size, body) => {
    if (err)
      return callback(err);
