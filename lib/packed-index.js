    ...
    async.waterfall([
      ...
      (buf, callback) => {
        ...
        if (type === 'ref_delta') {
          ...
          extra = binarySearch(idx, extra, lookupCompare);
          if (extra < 0) {
            return callback(
...
PackedIndex.prototype.resolveDelta = function resolveDelta(content, idx, off,
  ...
  async.waterfall([
    ...
    let patched;
    try {
      patched = delta.patch(result.parent[1], result.delta);
    } catch (e) {
      return callback(e);
    }
    ...
    body.end(patched);
    callback(null, result.parent[0].split(' ', 1)[0] + ' ' + patched.length,
             body);
  });
};
